...
export class Viewer {
    ...
    getPDFViewerContent(pdfFile: string): string {
        ...
        const url = `http://127.0.0.1:${this.extension.server.port}/viewer.html?incode=1&file=${encodePathWithPrefix(pdfFile)}`
        ...
        return `
            ...
            window.addEventListener('message', (e) => {
                if (e.origin !== 'http://127.0.0.1:${this.extension.server.port}') {
                    return;
                }
                switch (e.data.type) {
                    case 'initialized': {
